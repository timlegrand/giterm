...
class Postponer(object):
    ...
    def set(self, timeout_in_seconds=None, action=None, *args, **kwargs):
        ...
        If called again before timeout, registers new instructions and resets timer.
        '''
        self.timer.cancel()
...
if __name__ == '__main__':
    ...
    def no_do_that_instead(*args, **kwargs):
        print 'no_do_that_instead() executed with: %s' % print_args(args, kwargs)

    p = Postponer(timeout_in_seconds=1.0)  # does not start the timer
    p.set()  # start timer for one second
    time.sleep(0.5)
    p.set(action=please_work)  # new callback, timer reset
    time.sleep(0.5)
    p.set(timeout_in_seconds=0.6, args=['arg#1', 'arg#2'], kwargs={'file': 'this'})  # new arguments, timer reset
    time.sleep(1.5)
    p.set(timeout_in_seconds=0.6, action=no_do_that_instead)  # ditto
    time.sleep(0.5)
    p.set(timeout_in_seconds=0.6)  # timer reset
    time.sleep(0.7)
    p.set(timeout_in_seconds=0.6, args=['no this file instead'])  # new arguments, timer reset
    time.sleep(0.5)
